...
"""
def purchaseShopping(houses, items, availability, stores_dictionary, store_char, shopping_list, shopping_day, calc, items_to_buy, houses_to_buy_for):
    for x in houses_to_buy_for:
        for y in items_to_buy: 
            quantity = 0        
            if week_orders[y][x] > "0":
                if availability[y][stores_dictionary[store_char]] == "Y":
...
def main():
    ...
    data_file_one = importData("data_file_one", "DADSA CWK SHOPPING DATA WEEK 1 File A.csv", "file_one")
    data_file_two = importData("data_file_two", "DATA CWK SHOPPING DATA WEEK 1 FILE B.csv", "file_two")
    ...
    global monday_shopping
    global tuesday_shopping
    global wednesday_shopping
    global thursday_shopping
    global friday_shopping
    monday_shopping = shopping_list.copy()
    tuesday_shopping = shopping_list.copy()
    wednesday_shopping = shopping_list.copy()
    thursday_shopping = shopping_list.copy()
    friday_shopping = shopping_list.copy()

    #Create list showing the item availability of each store.
    availability = sortData(data_file_one, "availability", 3, 6)

    #Create list of all houses/flats
    houses = sortHouses(data_file_two)

    #Create lists for each week's orders.
    week_name_one = sortData(data_file_two, "week_one_orders", 1, 8)
    week_name_two = sortData(data_file_two, "week_two_orders", 8, 15)
    week_name_one.pop(0)
    week_name_two.pop(0)
    ...
    """
    Create list of boolean values to use in isReadyForDelivery() to ensure deliveries
    aren't made to the same household more than once.
    """
    shop_completed = []
    ...
    """
    Create variables for the index of each store,
    in order of which store has the most items needed.
    """
    best_store_index = sorted_stores.index(sorted_stores[0])
    second_best_store_index = sorted_stores.index(sorted_stores[1])
    worst_store_index = sorted_stores.index(sorted_stores[2])

    worst_store_items = []

    needs_worst_store = []
    doesnt_need_worst_store = []

    """
    This for loop checks for which items are only available
    from the worst store and then appends them into a list.
    """
    for y in range(len(items)):
        if availability[y][best_store_index] != "Y" and availability[y][second_best_store_index] != "Y" and availability[y][worst_store_index] == "Y":
            worst_store_items.append(y)
            
    """
    This for loop checks for households which require the
    use of the worst store and appends them into a list, 
    and vice versa with those that only need to use the 
    two other stores.
    """       
    for y in worst_store_items:
        for x in range(len(houses)):
            if week_orders[y][x] > "0":
                needs_worst_store.append(x)
            else:
                doesnt_need_worst_store.append(x)

    """
    Display the required shopping for Monday. Visit the store with the least items 
    available (worst_store_char) and add items to shopping list (inventory) if 
    required. Also add to Monday's copy of shopping (monday_shopping) for ease of 
    displaying data later on.
    """
    purchaseShopping(houses, items, availability, stores_dictionary, worst_store_char, shopping_list, monday_shopping, "1", worst_store_items,
    needs_worst_store)
    ...
    #For each house, see if they are ready for delivery after Monday's shopping.
    for x in range(len(houses)):
        isReadyForDelivery(x, items, week_orders, shopping_list, houses, total_needed[x], "Monday", shop_completed)

    remove_shopping = "int(week_orders[y][x]) - int(monday_shopping[items[y]])"

    """
    Complete shopping for Tuesday, visiting the best store, while only
    completing the shopping for the households that will need to buy 
    item(s) from the "worst store".
    """
    purchaseShopping(houses, items, availability, stores_dictionary, best_store_char, shopping_list, tuesday_shopping,
     remove_shopping, range(len(items)), needs_worst_store)

    #Check if houses are ready for delivery after Tuesday's shopping.
    for x in range(len(houses)):
        isReadyForDelivery(x, items, week_orders, shopping_list, houses, total_needed[x], "Tuesday", shop_completed)

    remove_shopping += " - int(tuesday_shopping[items[y]])"

    """
    Complete shopping for Wednesday, visiting the second best store,
    while only completing the shopping for the households that will
    need to buy item(s) from the "worst store".
    """
    purchaseShopping(houses, items, availability, stores_dictionary, second_best_store_char, shopping_list, wednesday_shopping,
     remove_shopping, range(len(items)), needs_worst_store)

    """
    Make final deliveries after Wednesday's shopping, should deliver to
    all houses that required the "worst shop".
    """
    for x in range(len(houses)):
        isReadyForDelivery(x, items, week_orders, shopping_list, houses, total_needed[x], "Wednesday", shop_completed)

    remove_shopping += " - int(wednesday_shopping[items[y]])"

    """
    Complete shopping for Thursday, visiting the best store,
    while completing the shopping for the households that don't
    need to buy item(s) from the "worst store".
    """
    purchaseShopping(houses, items, availability, stores_dictionary, best_store_char, shopping_list, thursday_shopping,
     "1", range(len(items)), doesnt_need_worst_store)

    #Check if houses are ready for delivery after Thursday's shopping.
    for x in range(len(houses)):
        isReadyForDelivery(x, items, week_orders, shopping_list, houses, total_needed[x], "Thursday", shop_completed)

    remove_shopping = "int(week_orders[y][x]) - int(thursday_shopping[items[y]])"

    """
    Complete shopping for Friday, visiting the second best store,
    while completing the shopping for the households that don't
    need to buy item(s) from the "worst store".
    """
    purchaseShopping(houses, items, availability, stores_dictionary, second_best_store_char, shopping_list, friday_shopping,
     remove_shopping, range(len(items)), doesnt_need_worst_store)

    #Complete final deliveries are Friday's shopping.
    for x in range(len(houses)):
        isReadyForDelivery(x, items, week_orders, shopping_list, houses, total_needed[x], "Friday", shop_completed)
    ...
    if day_selected == "6":
        ...
        displayDay(worst_store_char, monday_shopping, "Monday")
    elif day_selected == "2":
        displayDay(best_store_char, tuesday_shopping, "Tuesday")
    elif day_selected == "3":
        displayDay(second_best_store_char, wednesday_shopping, "Wednesday")
    elif day_selected == "4":
        displayDay(best_store_char, thursday_shopping, "Thursday")
    elif day_selected == "5":
        displayDay(second_best_store_char, friday_shopping, "Friday")
    else:
